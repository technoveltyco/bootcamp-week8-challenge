...
(function (global, localStorage, moment) {
  ...
  searchBtn.addEventListener("click", async function (event) {
    ...
    if (geolocationFound) {
      ...
      [today, forecast] = await fetchForecast(...geolocationFound);
  ...
  function displayToday(weatherObject) {
    ...
    todaySection.innerHTML = `
      <div class="card-wrapper weather today">
        <div class="card">
          <div class="card-body">
            <h2 class="card-title">
            ...
            <div class="measure temperature">
              <span class="label">Temp</span>:&nbsp;<span class="value">${weatherObject.temperature.avg}</span>&nbsp;<span class="unit">&#8451;C</span>
            </div>
            <div class="measure wind">
              <span class="label">Wind</span>:&nbsp;<span class="value">${weatherObject.wind.speed}</span>&nbsp;<span class="unit">KPH</span>
            </div>
            <div class="measure humidity">
              <span class="label">Humidity</span>:&nbsp;<span class="value">${weatherObject.humidity}</span>&nbsp;<span class="unit">%</span>
            </div>
          </div>
        </div>
      </div>  
  ...
  function displayForecast(weatherObjectList) {
    ...
    weatherObjectList.forEach((weatherObject, index) => {
      ...
      ctn.className = `card-wrapper weather forecast forecast-${index}`;
  ...
  async function run() {
    try {
      ...
      if (geolocation) {
        ...
        // Fetches weather today and 5 days forecasts data objects.
        const [today, forecast] = await fetchForecast(lat, lon);
